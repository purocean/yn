...
# Yank-Note Characteristic Function Instructions

[toc]{type: "ol", level: [2]}

## Application Data

The application-related data directory is stored under `<home>/yank-note`, click the "Open Home Directory" interface in the tray menu to view.
...
Write `[toc]{type: "ul", level: [1,2,3]}` to generate a directory  
You can control directory style (`ul` or `ol`) and level

## To-do Switch

Tick the list in preview area to try
...
2. The encryption and decryption processes are both completed at the front end.
...
Just need to add `{.mindmap}` to the root node of the list.

+ central node{.mindmap}
    + Principle of State Visibility 
    + Principle of Environmental Appropriate 
    + Principle of User Controllable 
    + Principle of Consistency  
    + Principle of Error Proofing 
    + Principle of Accessibility 
    + Principle of Agility and Efficiency 
    + Principle of Grace and Simplicity 
    + Principle of Fault Tolerance
    + Principle of Friendly Help 
...
<div>
    ...
    <button onclick="var x = document.getElementById('output'); x.value = x.value.toUpperCase()">Convert the value to uppercase</button>
</div>
```
...
::: group-item *Tab 2
test 2
A * before the title indicates that this tab is activated by default
:::
...
- qualifier escape:  [= '[\= =\]' =]
- Arithmetic:  [= (1 + 2) / 2 =]
- reference file (not support nested macro, you can use Front Matter variable defined in the target document): 
...
`$include` | (key: string, val: any) => Result | Define a variable that can be used in this document
