...
const data: BaseLanguage = {
  ...
  'external-file-readonly': {
    'readonly-desc': 'Внешние файлы по умолчанию доступны только для чтения. Вы можете [включить редактирование текущего документа](%s) или [изменить настройки](%s).',
    'enable-edit': 'Включить редактирование текущего документа',
    'open-setting': 'Открыть настройки',
  },
  ...
  'setting-panel': {
    ...
    'schema': {
      ...
      'editor': {
        ...
        'external-file-readonly_desc': 'При открытии внешних файлов (файлов вне репозитория) по умолчанию они доступны только для чтения',
      },
      'render': {
