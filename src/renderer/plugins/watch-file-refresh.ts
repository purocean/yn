...
export default {
  ...
  register: (ctx) => {
    ...
    async function startWatch (doc: Doc) {
      ...
      const watchHandler = await ctx.api.watchFile(repo, path, { awaitWriteFinish: { stabilityThreshold: 500, pollInterval: 50 }, alwaysStat: true }, payload => {
      ...
      watchHandler.result.catch((e: Error) => {
        if (!e.message.includes('abort')) {
          logger.error('startWatch error', e)
          // retry
          ctx.utils.sleep(2000).then(() => {
